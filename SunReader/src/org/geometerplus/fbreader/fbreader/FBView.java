...
//		), x, y);
		...
		myReader.runAction(ActionCode.SHOW_ACTIONBAR);
	...
	public ZLColor getTextColor(ZLTextHyperlink hyperlink) {
		...
		switch (hyperlink.Type) {
			default:
			case FBHyperlinkType.FOOTER:
				return profile.FooterTextOption.getValue();
	...
	private class Footer implements FooterArea {
		...
		public synchronized void paint(ZLPaintContext context) {
			...
			final ZLResource resource = ZLResource.resource("footNotePage");
			if (reader.FooterShowProgressOption.getValue()) {
				info.append(pagePosition.Current);
				info.append("/");
				info.append(pagePosition.Total).append(resource.getValue());
			}
			...
			final StringBuilder infoleft = new StringBuilder();
			int x = 100*pagePosition.Current/pagePosition.Total;
			int y = 1000*pagePosition.Current/pagePosition.Total-x*10;
						
			infoleft.append(x).append('.').append(y).append('%');
			String infoleftString = infoleft.toString();
			
			// draw info text
			context.setTextColor(footerColor);
			context.drawString(right - infoWidth, 12, infoString);

			// draw info text left
			context.drawString(left , 12, infoleftString);
